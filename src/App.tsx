...
import { KeyRound, Lock, FileSignature, ShieldCheck, Copy, Check, Upload, Download, LogIn, Info } from 'lucide-react';
import { cn } from './lib/utils';
import { generateRSAKeys, encryptPKCS1, decryptPKCS1, signPKCS1, verifyPKCS1 } from './lib/rsa';

type Tab = 'keygen' | 'encryption' | 'signature';
...
function SignatureModule({ defaultPublicKey, defaultPrivateKey }: { defaultPublicKey: string, defaultPrivateKey: string }) {
  ...
  const handleSign = () => {
    setErrorSign('');
    if (!privKey || !docToSign) {
      setErrorSign('Vui lòng nhập đầy đủ Private Key và Dữ liệu hợp đồng/văn bản cần ký.');
      return;
    }
    ...
    try {
      const result = signPKCS1(docToSign, privKey);
      setSignatureOutput(result);
      // Auto-fill bottom section
      setDocToVerify(docToSign);
  ...
  const handleVerify = () => {
    setErrorVerify('');
    setVerificationResult(null);
    if (!pubKey || !docToVerify || !signatureToVerify) {
      setErrorVerify('Cần tham số: Public Key gốc, Văn bản cần kiểm tra, và đoạn mã Chữ ký số Base64.');
      return;
    }
    ...
    try {
      const isValid = verifyPKCS1(docToVerify, signatureToVerify, pubKey);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-1 h-full relative">
            ...
            <textarea 
              ...
              className={cn(
                "flex-1 w-full min-h-[250px] p-4 text-xs font-mono leading-relaxed bg-indigo-900/5 border border-indigo-200 rounded-xl outline-none resize-none shadow-inner",
                signatureOutput ? "text-indigo-800" : "text-indigo-900/30"
              )}
              placeholder="Chữ ký duy nhất kết hợp giữa tài liệu và khóa bí mật sẽ xuất hiện..."
            />
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea 
                ...
                placeholder="Dán chữ ký đính kèm kèm tài liệu vào đây..."
              />
            </div>
            <button 
